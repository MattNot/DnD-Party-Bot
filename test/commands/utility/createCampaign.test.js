...
import command from '../../../commands/utility/createCampaign.js'; // Assicurati che il percorso sia corretto
...
await test('top level test for createCampaign', async (t) => {
  await t.test('should connect to MongoDB and update the collection', async () => {
    ...
    // InsertOne has to be called once
    assert.deepStrictEqual(mockMongoClient.db().collection().insertOne.mock.callCount(), 1);
